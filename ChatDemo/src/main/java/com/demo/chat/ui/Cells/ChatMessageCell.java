...
import com.demo.chat.ui.Components.CheckBoxBase;
import com.demo.chat.ui.Components.CubicBezierInterpolator;
import com.demo.chat.ui.Components.EmptyStubSpan;
...
import com.demo.chat.ui.Components.RoundVideoPlayingDrawable;
...
public class ChatMessageCell extends BaseCell implements
        SeekBar.SeekBarDelegate,
    ...
    public interface ChatMessageCellDelegate {
        ...
        default void didPressChannelAvatar(ChatMessageCell cell, Chat chat, int postId, float touchX, float touchY) {
        }
        ...
        default void didPressBotButton(ChatMessageCell cell, KeyboardButton button) {
        }

        default void didPressReaction(ChatMessageCell cell, ReactionCount reaction) {
        }
    ...
    private static class BotButton {
        ...
        private ReactionCount reaction;
        private int angle;
    ...
    private Media photoParentObject;
    ...
    public void updatePlayingMessageProgress() {
        ...
        if (documentAttachType == DOCUMENT_ATTACH_TYPE_VIDEO) {
            ...
            int duration = 0;
            Document document = currentMessageObject.getDocument();
            ...
            if (documentAttachType == DOCUMENT_ATTACH_TYPE_AUDIO) {
                if (!MediaController.getInstance().isPlayingMessage(currentMessageObject)) {
                    for (int a = 0; a < documentAttach.attributes.size(); a++) {
                        Document.DocumentAttribute attribute = documentAttach.attributes.get(a);
    ...
    private boolean isPhotoDataChanged(MessageObject object) {
        ...
        if (object.type == 4) {
            ...
            if (object.messageOwner.media.isGeoLive()) {
        ...
        } else if (currentPhotoObject == null || currentPhotoObject.location ==null) {
    ...
    private boolean isUserDataChanged() {
        if (currentMessageObject != null && (!hasLinkPreview && currentMessageObject.messageOwner.media != null && currentMessageObject.messageOwner.media.webpage instanceof MessageMedia.WebPage)) {
            return true;
        }
        ...
        if (replyNameLayout != null && currentMessageObject.replyMessageObject != null) {
            PhotoSize photoSize = FileLoader.getClosestPhotoSizeWithSize(currentMessageObject.replyMessageObject.photoThumbs, 40);
            ...
            if (messageObject.type == 0) {
                ...
                if (AndroidUtilities.isTablet()) {
                    if (isChat && !messageObject.isOutOwner() && messageObject.needDrawAvatar()) {
                        ...
                        drawName = messageObject.messageOwner.to_id != 0 && !messageObject.isOutOwner();
                ...
                hasGamePreview = false;
                hasInvoicePreview = messageObject.messageOwner.media.isInvoice();
                hasLinkPreview = messageObject.messageOwner.media.isWebPage() && messageObject.messageOwner.media.webpage != null;
...
//                            ArrayList<Document> documents = attribute.documents;
...
//                        }
                    ...
                    if ((siteName.equals("instagram")
                            || siteName.equals("twitter") 
                            || "telegram_album".equals(webpageType)) 
                            && messageObject.messageOwner.media.webpage.cached_page instanceof Page 
                            && (messageObject.messageOwner.media.webpage.photo instanceof MessageMedia.Photo
                            || MessageObject.isVideoDocument(messageObject.messageOwner.media.webpage.document))) {
                        drawInstantView = false;
                        slideshow = true;
                        ...
                        for (int a = 0; a < blocks.size(); a++) {
                            ...
                            if (block instanceof Block.TL_pageBlockSlideshow) {
                                Block.TL_pageBlockSlideshow b = (Block.TL_pageBlockSlideshow) block;
                                count = b.items.size();
                            } else if (block instanceof Block.TL_pageBlockCollage) {
                                Block.TL_pageBlockCollage b = (Block.TL_pageBlockCollage) block;
                                count = b.items.size();
                            }
                        }
                ...
                if (hasLinkPreview || hasGamePreview || hasInvoicePreview) {
                    ...
                    String type=null;
                    if (hasLinkPreview) {
                        MessageMedia.WebPage webPage = (MessageMedia.WebPage) messageObject.messageOwner.media.webpage;
                        ...
                        MessageMedia invoice = (MessageMedia) messageObject.messageOwner.media;
                        ...
                        author = null;
                        document = null;
//                        if (invoice.photo instanceof WebDocument) {
...
//                        }
                    ...
                    if (document != null) {
                        if (MessageObject.isRoundVideoDocument(document)) {
                            ...
                            if (currentPhotoObject != null && (currentPhotoObject.w == 0 || currentPhotoObject.h == 0 || currentPhotoObject !=null)) {
                                for (int a = 0; a < document.attributes.size(); a++) {
                                    Document.DocumentAttribute attribute = document.attributes.get(a);
                                    if (attribute.isVideo()) {
                                        if (currentPhotoObject !=null) {
                    ...
                    if (documentAttachType != DOCUMENT_ATTACH_TYPE_MUSIC
                        ...
                        if (currentPhotoObject != null
                            ...
                            } else if (currentPhotoObject !=null && currentPhotoObject.w != 0) {
                                ...
                                if (documentAttachType == DOCUMENT_ATTACH_TYPE_WALLPAPER) {
                                    ...
                                    if (SharedConfig.autoplayVideo && (
                                        ...
                                        if (currentPhotoObjectThumb != null) {
                                            ...
                                            photoImage.setImage(null, null, ImageLocation.getForObject(currentPhotoObject, photoParentObject), currentPhotoObject !=null || "s".equals(currentPhotoObject.type) ? currentPhotoFilterThumb : currentPhotoFilter, 0, null, messageObject, 0);
                                        }
                                    }
                                    ...
                                    String filter = currentPhotoObject !=null || "s".equals(currentPhotoObject.type) ? currentPhotoFilterThumb : currentPhotoFilter;
                ...
                CharSequence currentNameString = UserObject.formatName(messageObject.messageOwner.media.first_name, messageObject.messageOwner.media.last_name).replace('\n', ' ');
                ...
                if (messageObject.type == 9) {
                    ...
                    MessageMedia.GeoPoint point = messageObject.messageOwner.media.geo;
                    ...
                    if (messageObject.messageOwner.media.isGeoLive()) {
                        if (AndroidUtilities.isTablet()) {
                    ...
                    for (int a = 0; a < messageObject.getDocument().attributes.size(); a++) {
                        Document.DocumentAttribute attribute = messageObject.getDocument().attributes.get(a);
                        if (attribute.isImageSize()) {
                    ...
                    if (messageObject.type == MessageObject.TYPE_ROUND_VIDEO) {
                        ...
                        PhotoSize size = currentPhotoObject != null ? currentPhotoObject : currentPhotoObjectThumb;
                        int imageW = 0;
                        int imageH = 0;
                        if (size != null) {
                            imageW = size.w;
            ...
            if (currentPosition == null && (messageObject.messageOwner.reply_markup.isInlineMarkup() || messageObject.messageOwner.reactions != null && !messageObject.messageOwner.reactions.results.isEmpty())) {
                ...
                if (messageObject.messageOwner.reply_markup.isInlineMarkup()) {
                    for (int a = 0; a < rows; a++) {
                        ReplyMarkup.KeyboardButtonRow row = messageObject.messageOwner.reply_markup.rows.get(a);
                    ...
                    for (int b = 0; b < buttonsCount; b++) {
                        ReactionCount reaction = messageObject.messageOwner.reactions.results.get(b);
                        BotButton botButton = new BotButton();
            ...
            if (documentAttachType == DOCUMENT_ATTACH_TYPE_MUSIC) {
                if (MessageObject.isDocumentHasThumb(documentAttach)) {
                    PhotoSize thumb = FileLoader.getClosestPhotoSizeWithSize(documentAttach.thumbs, 90);
    ...
    protected void onLongPress() {
        ...
        if (pressedVoteButton != -1 || psaHintPressed || instantPressed) {
            instantPressed = instantButtonPressed = false;
            pressedVoteButton = -1;
            psaHintPressed = false;
            if (Build.VERSION.SDK_INT >= 21 && selectorDrawable != null) {
    ...
    //region SeekBar.SeekBarDelegate
    @Override
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (changed || !wasLayout) {
            ...
            if ((currentMessageObject.messageOwner.flags & Message.MESSAGE_FLAG_HAS_VIEWS) != 0) {
    ...
    private void drawContent(Canvas canvas) {
        ...
        if (currentMessageObject.type == 4 && !(currentMessageObject.messageOwner.media.isGeoLive()) && currentMapProvider == 2 && photoImage.hasNotThumb()) {
        ...
        if (!botButtons.isEmpty()) {
            ...
            for (int a = 0; a < botButtons.size(); a++) {
                ...
                canvas.restore();
                if (button.button.isKeyboardButtonUrl()) {
                ...
                } else if (button.button.isSwitchInline()) {
                ...
                } else if (button.button.isButtonCallback() || button.button.isRequestGeoLocation()
                         || button.button.isButtonUrlAuth()) {
                    boolean drawProgress = (button.button.isButtonCallback()
                            || button.button.isButtonUrlAuth())
                            && SendMessagesHelper.getInstance(currentAccount).isSendingCallback(currentMessageObject, button.button)
                            || button.button.isRequestGeoLocation()
                            && SendMessagesHelper.getInstance(currentAccount).isSendingCurrentLocation(currentMessageObject, button.button);
        ...
        if (replyNameLayout != null) {
            if (currentMessageObject.shouldDrawWithoutBackground()) {
                ...
                if (currentMessageObject.isOutOwner()) {
                    ...
                    if (currentMessageObject.hasValidReplyMessageObject() && currentMessageObject.replyMessageObject.type == 0 && !currentMessageObject.replyMessageObject.messageOwner.media.isInvoice()) {
    ...
    //region draw DrawSelectionBackground
    ...
    private void measureTime(MessageObject messageObject) {
        ...
        if (messageObject.scheduled) {
            ...
            User signUser = MessagesController.getInstance(currentAccount).getUser(messageObject.messageOwner.from_id);
            if (signUser != null) {
                signString = UserObject.formatName(signUser.first_name, signUser.last_name).replace('\n', ' ');
            } else {
                signString = null;
            }
        } else {
        ...
        if (messageObject.scheduled || messageObject.isLiveLocation() || messageObject.messageOwner.edit_hide || messageObject.getDialogId() == 777000 || messageObject.messageOwner.via_bot_id != 0 || messageObject.messageOwner.via_bot_name != null || author != null && author.bot) {
            ...
            edited = (messageObject.messageOwner.flags & Message.MESSAGE_FLAG_EDITED) != 0 || messageObject.isEditing();
        } else {
            edited = false;
            for (int a = 0, size = currentMessagesGroup.messages.size(); a < size; a++) {
                MessageObject object = currentMessagesGroup.messages.get(a);
                if ((object.messageOwner.flags & Message.MESSAGE_FLAG_EDITED) != 0 || object.isEditing()) {
                    edited = true;
                    break;
                }
            }
        ...
        if ((messageObject.messageOwner.flags & Message.MESSAGE_FLAG_HAS_VIEWS) != 0) {
        ...
        if (messageObject.scheduled) {
            ...
            } else if (messageObject.isSending() && messageObject.messageOwner.to_id != 0 && !messageObject.isMegagroup()) {
                timeWidth += AndroidUtilities.dp(18);
            }
        }
        if (signString != null) {
    ...
    public void drawTime(Canvas canvas, float alpha) {
        ...
        if (mediaBackground && captionLayout == null) {
            ...
            if (ChatObject.isChannel(currentChat) && !currentChat.megagroup || (currentMessageObject.messageOwner.flags & Message.MESSAGE_FLAG_HAS_VIEWS) != 0) {
    ...
    public void drawOverlays(Canvas canvas) {
        ...
        if (currentMessageObject.type == MessageObject.TYPE_VIDEO
            ...
            if (docTitleLayout != null) {
                ...
                if (currentMessageObject.messageOwner.media.isGeoLive()) {
        ...
        if (buttonState == 0 && (!drawVideoImageButton || video)) {
            if (documentAttachType == DOCUMENT_ATTACH_TYPE_AUDIO || documentAttachType == DOCUMENT_ATTACH_TYPE_MUSIC) {
                ...
                if (currentPhotoObject != null && (photoImage.hasNotThumb() || currentPhotoObjectThumb == null)) {
                    ...
                    thumbFilter = thumb !=null || "s".equals(thumb.type) ? currentPhotoFilterThumb : currentPhotoFilter;
                } else {
    ...
    //region ImageReceiver.ImageReceiverDelegate
    @Override
            ...
            if (drawable instanceof RLottieDrawable) {
                ...
                if (stickerSet != null) {
                    ...
                    if (value >= 0 && value < stickerSet.documents.size()) {
                        ...
                        Document document = stickerSet.documents.get(Math.max(value, 0));
                        File path = FileLoader.getPathToAttach(document, true);
    ...
    //region DownloadController.FileDownloadProgressListener
    @Override
    ...
    public void onSuccessDownload(String fileName) {
        if (documentAttachType == DOCUMENT_ATTACH_TYPE_STICKER && currentMessageObject.isDice()) {
            ...
            if (!currentMessageObject.needDrawBluredPreview() && !autoPlayingMedia && documentAttach != null) {
                if (documentAttachType == DOCUMENT_ATTACH_TYPE_ROUND) {
                    photoImage.setImage(ImageLocation.getForDocument(documentAttach), ImageLoader.AUTOPLAY_FILTER, ImageLocation.getForObject(currentPhotoObject, photoParentObject), currentPhotoObject !=null || currentPhotoObject != null && "s".equals(currentPhotoObject.type) ? currentPhotoFilterThumb : currentPhotoFilter, ImageLocation.getForObject(currentPhotoObjectThumb, photoParentObject), currentPhotoFilterThumb, null, documentAttach.size, null, currentMessageObject, 0);
                    photoImage.setAllowStartAnimation(true);
                    photoImage.startAnimation();
                    autoPlayingMedia = true;
    ...
    //region LoadingProgressLayout
    private void createLoadingProgressLayout(Document document) {
        if (document == null) {
            return;
        }
    ...
    private boolean isOpenChatByShare(MessageObject messageObject) {
        return messageObject.messageOwner.fwd_from != null && messageObject.messageOwner.fwd_from.saved_from_chat_id != 0;
    }

    private boolean checkNeedDrawShareButton(MessageObject messageObject) {
        if ((currentPosition != null && !currentPosition.last) || currentMessageObject.deleted) {
            return false;
        } else if (messageObject.messageOwner.fwd_from != null && !messageObject.isOutOwner() && messageObject.messageOwner.fwd_from.saved_from_chat_id != 0 && messageObject.getDialogId() == UserConfig.getInstance(currentAccount).getClientUserId()) {
    ...
    private void updateCurrentUserAndChat() {
        ...
        MessageFwdHeader fwd_from = currentMessageObject.messageOwner.fwd_from;
        ...
        if (fwd_from != null && fwd_from.channel_id != 0 && currentMessageObject.getDialogId() == currentUserId) {
        ...
        } else if (fwd_from != null && fwd_from.saved_from_chat_id != 0) {
            currentChat = messagesController.getChat(fwd_from.saved_from_chat_id);
            ...
            currentChat = messagesController.getChat(currentMessageObject.messageOwner.to_id);
        }
    }

    private void setMessageObjectInternal(MessageObject messageObject) {
        if ((messageObject.messageOwner.flags & Message.MESSAGE_FLAG_HAS_VIEWS) != 0 && !currentMessageObject.scheduled) {
...
//                MessagesController.getInstance(currentAccount).addToViewsQueue(currentMessageObject);
        ...
        if (messageObject.hasValidReplyMessageObject() || messageObject.messageOwner.fwd_from != null && messageObject.isDice()) {
            if (currentPosition == null || currentPosition.minY == 0) {
                ...
                if (messageObject.hasValidReplyMessageObject()) {
                    ...
                    PhotoSize thumbPhotoSize = FileLoader.getClosestPhotoSizeWithSize(messageObject.replyMessageObject.photoThumbs2, 40);
                    ...
                    if (messageObject.customReplyName != null) {
                        ...
                        if (messageObject.replyMessageObject.isFromUser()) {
                            User user = MessagesController.getInstance(currentAccount).getUser(messageObject.replyMessageObject.messageOwner.from_id);
                            if (user != null) {
                                name = UserObject.getUserName(user);
                            }
                        } else if (messageObject.replyMessageObject.messageOwner.from_id < 0) {
                            Chat chat = MessagesController.getInstance(currentAccount).getChat(-messageObject.replyMessageObject.messageOwner.from_id);
                            if (chat != null) {
                                name = chat.title;
                            }
                        } else {
                            Chat chat = MessagesController.getInstance(currentAccount).getChat(messageObject.replyMessageObject.messageOwner.to_id);
                            if (chat != null) {
                                name = chat.title;
                            }
                        }
                    ...
                    if (messageObject.replyMessageObject.messageOwner.media.isInvoice()) {
    ...
    public Document getStreamingMedia() {
        return documentAttachType == DOCUMENT_ATTACH_TYPE_VIDEO
                       || documentAttachType == DOCUMENT_ATTACH_TYPE_ROUND
                       || documentAttachType == DOCUMENT_ATTACH_TYPE_GIF ? documentAttach : null;
    }
