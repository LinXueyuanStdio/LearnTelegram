        ...
        try {
            if (scheduled) {
                ...
                for (int a = 0; a < messages.size(); a++) {
                    ...
                    if (!(message.action.isHistoryClear()) && (!MessageObject.isOut(message) || message.from_scheduled) && (message.id > 0 || MessageObject.isUnread(message))) {
                ...
                for (int a = 0; a < messages.size(); a++) {
                    ...
                    if (downloadMask != 0 && (message.to_id == 0 || message.post) && message.date >= getConnectionsManager().getCurrentTime() - 60 * 60 && getDownloadController().canDownloadMedia(message) == 1) {
                        if (message.media.isPhoto() || message.media.isDocument() || message.media.isWebPage()) {
                            int type = 0;
                            long id = 0;
                            MessageMedia object = null;
                            Document document = MessageObject.getDocument(message);
                            MessageMedia.Photo photo = MessageObject.getPhoto(message);
                            if (MessageObject.isVoiceMessage(message)) {
                                id = document.id;
                                type = DownloadController.AUTODOWNLOAD_TYPE_AUDIO;
                                object = new MessageMedia().setDocument(true);
                                ...
                                if (photoSize != null) {
                                    id = photo.id;
                                    type = DownloadController.AUTODOWNLOAD_TYPE_PHOTO;
                                    object = new MessageMedia().setPhoto(true);
                                    object.photo = photo;
                                    object.flags |= 1;
                                    if (message.media.isWebPage()) {
