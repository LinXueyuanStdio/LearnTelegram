...
import com.demo.chat.ui.Cells.StickerSetGroupInfoCell;
...
public class EmojiView extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
    ...
    private ArrayList<MessagesStickerSet> stickerSets = new ArrayList<>();
    private int groupStickerPackNum;
    private int groupStickerPackPosition;
    private boolean groupStickersHidden;
    private MessagesStickerSet groupStickerSet;
    ...
    private StickerSetCovered[] primaryInstallingStickerSets = new StickerSetCovered[10];
    ...
    public interface EmojiViewDelegate {
        ...
        default void onStickerSelected(View view, Document sticker, Object parent, boolean notify, int scheduleDate) {
        ...
        default void onShowStickerSet(StickerSet stickerSet, InputStickerSet inputStickerSet) {

        }

        default void onStickerSetAdd(StickerSetCovered stickerSet) {

        }

        default void onStickerSetRemove(StickerSetCovered stickerSet) {
    ...
    public EmojiView(boolean needStickers, boolean needGif, final Context context, boolean needSearch, final Chat chatFull) {
        super(context);
        ...
        if (needStickers) {
                ...
                public int getSpanSize(int position) {
                    if (stickersGridView.getAdapter() == stickersGridAdapter) {
                        ...
                        if (position != stickersGridAdapter.totalItems) {
                            ...
                            if (object == null || stickersGridAdapter.cache.get(position) instanceof Document) {
                                return 1;
                            }
                        }
                        return stickersGridAdapter.stickersPerRow;
                        ...
                        if (position != stickersSearchGridAdapter.totalItems) {
                            ...
                            if (object == null || stickersSearchGridAdapter.cache.get(position) instanceof Document) {
                                return 1;
                            }
                        }
                        return stickersGridAdapter.stickersPerRow;
            ...
            stickersOnItemClickListener = (view, position) -> {
                if (stickersGridView.getAdapter() == stickersSearchGridAdapter) {
                    StickerSetCovered pack = stickersSearchGridAdapter.positionsToSets.get(position);
                    if (pack != null) {
        ...
        bottomTabContainer.addView(bottomTabContainerBackground, new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT, AndroidUtilities.dp(44), Gravity.LEFT | Gravity.BOTTOM));
    ...
    public void closeSearch(boolean animated, long scrollToSet) {
        ...
        if (currentItem == 2 && scrollToSet != -1) {
            MessagesStickerSet set = MediaDataController.getInstance(currentAccount).getStickerSetById(scrollToSet);
            if (set != null) {
    ...
    private void updateStickerTabs() {
        ...
        ArrayList<MessagesStickerSet> packs = mediaDataController.getStickerSets(MediaDataController.TYPE_IMAGE);
        for (int i = 0; i < primaryInstallingStickerSets.length; i++) {
            final StickerSetCovered installingStickerSet = primaryInstallingStickerSets[i];
            if (installingStickerSet != null) {
                final MessagesStickerSet pack = mediaDataController.getStickerSetById(installingStickerSet.set.id);
                if (pack != null && !pack.set.archived) {
                    primaryInstallingStickerSets[i] = null;
                } else {
                    final MessagesStickerSet set = new MessagesStickerSet();
        ...
        for (int a = 0; a < packs.size(); a++) {
            MessagesStickerSet pack = packs.get(a);
        ...
        if (info != null) {
            ...
            Chat chat = MessagesController.getInstance(currentAccount).getChat(info.id);
            ...
            if (info.stickerset != null) {
                MessagesStickerSet pack = mediaDataController.getGroupStickerSetById(info.stickerset);
                if (pack != null && pack.documents != null && !pack.documents.isEmpty() && pack.set != null) {
                    MessagesStickerSet set = new MessagesStickerSet();
                ...
                MessagesStickerSet pack = new MessagesStickerSet();
                if (groupStickersHidden) {
                    groupStickerPackNum = stickerSets.size();
                    stickerSets.add(pack);
                } else {
    ...
    private void checkDocuments(boolean isGif) {
        if (isGif) {
            ...
            for (int a = 0; a < favouriteStickers.size(); a++) {
                Document favSticker = favouriteStickers.get(a);
                for (int b = 0; b < recentStickers.size(); b++) {
                    Document recSticker = recentStickers.get(b);
    ...
    public void showStickerBanHint(boolean gif) {
        ...
        Chat chat = MessagesController.getInstance(currentAccount).getChat(currentChatId);
        if (chat == null) {
            return;
        }
    ...
    private class StickersGridAdapter extends RecyclerListView.SelectionAdapter {
        ...
        public int getTabForPosition(int position) {
            ...
            if (pack instanceof String) {
                ...
                MessagesStickerSet set = (MessagesStickerSet) pack;
            ...
            switch (holder.getItemViewType()) {
                ...
                case 1: {
                    ...
                    if (position == totalItems) {
                        ...
                        if (row == Integer.MIN_VALUE) {
                            ...
                            Object pack = rowStartPack.get(row);
                            if (pack instanceof MessagesStickerSet) {
                                documents = ((MessagesStickerSet) pack).documents;
                ...
                case 2: {
                    ...
                    if (position == groupStickerPackPosition) {
                        ...
                        Chat chat = info != null ? MessagesController.getInstance(currentAccount).getChat(info.id) : null;
                        ...
                        Object object = cache.get(position);
                        if (object instanceof MessagesStickerSet) {
                            MessagesStickerSet set = (MessagesStickerSet) object;
                            if (set.set != null) {
        ...
        public void notifyDataSetChanged() {
            ...
            cache.clear();
            totalItems = 0;
            ArrayList<MessagesStickerSet> packs = stickerSets;
            int startRow = 0;
            for (int a = -3; a < packs.size(); a++) {
                ArrayList<Document> documents;
                MessagesStickerSet pack = null;
                String key;
                if (a == -3) {
...
//        }
    ...
    private class GifLayoutManager extends ExtendedGridLayoutManager {
        ...
        protected Size getSizeForItem(int i) {
            Document document;
            ArrayList<Document.DocumentAttribute> attributes;
            if (gifGridView.getAdapter() == gifAdapter) {
                if (i > gifAdapter.recentItemsCount) {
                    BotInlineResult result = gifAdapter.results.get(i - gifAdapter.recentItemsCount - 1);
                ...
                BotInlineResult result = gifSearchAdapter.results.get(i);
                document = result.document;
                if (document != null) {
                    attributes = document.attributes;
                } else if (result.content != null) {
        ...
        public Size getSizeForItem(Document document) {
            return getSizeForItem(document, document.attributes);
        }

        public Size getSizeForItem(Document document, List<Document.DocumentAttribute> attributes) {
    ...
    private class StickersSearchGridAdapter extends RecyclerListView.SelectionAdapter {
        ...
        private ArrayList<MessagesStickerSet> localPacks = new ArrayList<>();
        private HashMap<MessagesStickerSet, Boolean> localPacksByShortName = new HashMap<>();
        private HashMap<MessagesStickerSet, Integer> localPacksByName = new HashMap<>();
        ...
        private Runnable searchRunnable = new Runnable() {
            ...
            public void run() {
                ...
                ArrayList<MessagesStickerSet> local = MediaDataController.getInstance(currentAccount).getStickerSets(MediaDataController.TYPE_IMAGE);
                int index;
                for (int a = 0, size = local.size(); a < size; a++) {
                    MessagesStickerSet set = local.get(a);
...
//            }TODO 取消请求
        ...
        public int getItemViewType(int position) {
            ...
            if (object != null) {
                if (object instanceof Document) {
                    return 0;
                } else if (object instanceof StickerSetCovered) {
                    return 3;
                } else {
                    return 2;
                }
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            switch (holder.getItemViewType()) {
                ...
                case 1: {
                    ...
                    if (position == totalItems) {
                        ...
                        if (row == Integer.MIN_VALUE) {
                            ...
                            Integer count;
                            if (pack instanceof MessagesStickerSet) {
                                count = ((MessagesStickerSet) pack).documents.size();
                ...
                case 3: {
                    StickerSetCovered stickerSetCovered = (StickerSetCovered) cache.get(position);
        ...
        public void notifyDataSetChanged() {
            ...
            for (int a = -1, serverCount = serverPacks.size(), localCount = localPacks.size(), emojiCount = (emojiArrays.isEmpty() ? 0 : 1); a < serverCount + localCount + emojiCount; a++) {
                ...
                if (a == -1) {
                    ...
                    int idx = a;
                    if (idx < localCount) {
                        MessagesStickerSet set = localPacks.get(idx);
                        ...
                        if (idx < emojiCount) {
                            ...
                            for (int i = 0, N = emojiArrays.size(); i < N; i++) {
                                ...
                                for (int b = 0, size = documents.size(); b < size; b++) {
                                    ...
                                    if (a >= localCount && pack instanceof StickerSetCovered) {
                                        positionsToSets.put(num, (StickerSetCovered) pack);
                                    }
                                    documentsCount++;
                                }
                            }
                            ...
                            idx -= emojiCount;
                            StickerSetCovered set = serverPacks.get(idx);
                ...
                cache.put(totalItems, pack);
                if (a >= localCount && pack instanceof StickerSetCovered) {
                    positionsToSets.put(totalItems, (StickerSetCovered) pack);
                }
                positionToRow.put(totalItems, startRow);
                for (int b = 0, size = documents.size(); b < size; b++) {
                    int num = 1 + b + totalItems;
