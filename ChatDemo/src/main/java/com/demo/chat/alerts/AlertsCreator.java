...
import com.demo.chat.ui.Components.ShareLocationDrawable;
...
//            });
    ...
    public static void createClearOrDeleteDialogAlert(BaseFragment fragment, boolean clear, Chat chat, User user, boolean secret, MessagesStorage.BooleanCallback onProcessRunnable) {
        createClearOrDeleteDialogAlert(fragment, clear, false, false, chat, user, secret, onProcessRunnable);
    }

    public static void createClearOrDeleteDialogAlert(BaseFragment fragment, boolean clear, boolean admin, boolean second, Chat chat, User user, boolean secret, MessagesStorage.BooleanCallback onProcessRunnable) {
    ...
    public static void createBlockDialogAlert(BaseFragment fragment, int count, boolean reportSpam, User user, BlockDialogCallback onProcessRunnable) {
        ...
        String actionText;
        if (count == 1) {
            String name = UserObject.formatName(user.first_name, user.last_name);
    ...
    public static BottomSheet.Builder createScheduleDatePickerDialog(Context context, long dialogId, long currentDate, final ScheduleDatePickerDelegate datePickerDelegate, final Runnable cancelRunnable) {
        ...
        if ((int) dialogId > 0 && dialogId != selfUserId) {
            User user = MessagesController.getInstance(UserConfig.selectedAccount).getUser((int) dialogId);
...
//    }
    ...
    public static Dialog createLocationUpdateDialog(final Activity parentActivity, User user, final MessagesStorage.IntCallback callback) {
        final int[] selected = new int[1];
...
//            User u = UserConfig.getInstance(a).getCurrentUser();
//            if (u != null) {
    ...
    public static void createDeleteMessagesAlert(BaseFragment fragment, User user, Chat chat, long mergeDialogId, MessageObject selectedMessage, SparseArray<MessageObject>[] selectedMessages, MessageObject.GroupedMessages selectedGroup, boolean scheduled, int loadParticipant, Runnable onDelete) {
        ...
        if (chat != null && chat.megagroup && !scheduled) {
            ...
            if (selectedMessage != null) {
                if (selectedMessage.messageOwner.action == null || selectedMessage.messageOwner.action.isEmpty() ||
                        selectedMessage.messageOwner.action.isChatDeleteUser() ||
                        selectedMessage.messageOwner.action.isChatJoinedByLink() ||
                        selectedMessage.messageOwner.action.isChatAddUser()) {
                    actionUser = MessagesController.getInstance(currentAccount).getUser(selectedMessage.messageOwner.from_id);
                }
                boolean hasOutgoing = !selectedMessage.isSendError() && selectedMessage.getDialogId() == mergeDialogId && (selectedMessage.messageOwner.action == null
                        || selectedMessage.messageOwner.action.isEmpty())
                        && selectedMessage.isOut()
                        && (currentDate - selectedMessage.messageOwner.date) <= revokeTimeLimit;
                if (hasOutgoing) {
                    myMessagesCount++;
                }
            } else {
...
//                        progressDialog[0].setOnCancelListener(dialog -> ConnectionsManager.getInstance(currentAccount).cancelRequest(requestId, true));TODO 取消请求
                ...
                for (int a = 0; a < 3; a++) {
                    ...
                    if (a == 0) {
                        ...
                        cell.setText(LocaleController.formatString("DeleteAllFrom", R.string.DeleteAllFrom, UserObject.formatName(actionUser.first_name, actionUser.last_name)), "", false, false);
                    }
        ...
        } else if (!scheduled && !ChatObject.isChannel(chat)) {
            int currentDate = ConnectionsManager.getInstance(currentAccount).getCurrentTime();
            if (user != null && user.id != UserConfig.getInstance(currentAccount).getClientUserId() && !user.bot || chat != null) {
                if (selectedMessage != null) {
                    boolean hasOutgoing = !selectedMessage.isSendError()
                            ...
                            || selectedMessage.messageOwner.action .isEmpty()
                            || selectedMessage.messageOwner.action.isPhoneCall())
                            && (selectedMessage.isOut()
                            || canRevokeInbox
                    ...
                    for (int a = 1; a >= 0; a--) {
                        for (int b = 0; b < selectedMessages[a].size(); b++) {
                            MessageObject msg = selectedMessages[a].valueAt(b);
                            if (!(msg.messageOwner.action == null || msg.messageOwner.action.isEmpty() || msg.messageOwner.action.isPhoneCall())) {
                                continue;
                            }
        ...
        final User userFinal = actionUser;
