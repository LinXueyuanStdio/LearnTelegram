...
import com.demo.chat.ui.Cells.ChatLoadingCell;
import com.demo.chat.ui.Cells.ChatMessageCell;
import com.demo.chat.ui.Cells.ChatUnreadCell;
...
public class ChatActivity extends BaseFragment
    ...
    private final static int[] allowedNotificationsDuringChatListAnimations = new int[]{
            ...
            NotificationCenter.pinnedMessageDidLoad,
        ...
        public boolean onTouchEvent(MotionEvent event) {
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                if (textLayout != null) {
                    ...
                    int contentWidth = lineWidth + (circleWidth > 0 ? circleWidth + AndroidUtilities.dp(8) : 0);
                    ...
                    if (message != null) {
                        if (messageObject != null) {
                            ...
                            for (int b = 0; b < message.photoThumbs.size(); b++) {
                                PhotoSize photoSize = message.photoThumbs.get(b);
                                if (photoSize.location.volume_id == fileLocation.volume_id
                                        && photoSize.location.local_id == fileLocation.local_id) {
                                    imageReceiver = cell.getPhotoImage();
                                    break;
                                }
                            }
    ...
    private PhotoViewer.PhotoViewerProvider botContextProvider = new PhotoViewer.EmptyPhotoViewerProvider() {
        ...
        public void sendButtonPressed(int index, VideoEditedInfo videoEditedInfo, boolean notify, int scheduleDate) {
            ...
            sendBotInlineResult((BotInlineResult) botContextResults.get(index), notify, scheduleDate);
        }
    };
    ...
    public boolean onFragmentCreate() {
        ...
        if (chatId != 0) {
            ...
            //            }
            //        } else if (userId != 0) {
            //            currentUser = getMessagesController().getUser(userId);TODO
            //            dialog_id = userId;
            //            botUser = arguments.getString("botUser");
            //            if (inlineQuery != null) {
            //                getMessagesController().sendBotStart(currentUser, inlineQuery);TODO
            //            }
        } else {
            return false;
        }
        ...
        //region 3.加载聊天对象成功，开启媒体观察者和消息观察者
        ...
        if (startLoadFromMessageId != 0) {
            ...
            if (migrated_to != 0) {
                ...
                getMessagesController().loadMessages(mergeDialogId, 0, loadInfo, loadingFromOldPosition ? 50 : (AndroidUtilities.isTablet() ? 30 : 20), startLoadFromMessageId, 0, true, 0, classGuid, 3, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
            } else {
                getMessagesController().loadMessages(dialog_id, mergeDialogId, loadInfo, loadingFromOldPosition ? 50 : (AndroidUtilities.isTablet() ? 30 : 20), startLoadFromMessageId, 0, true, 0, classGuid, 3, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
            }
        } else {
            getMessagesController().loadMessages(dialog_id, mergeDialogId, loadInfo, AndroidUtilities.isTablet() ? 30 : 20, 0, 0, true, 0, classGuid, 2, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
        }
        if (!inScheduleMode) {
            waitingForLoad.add(lastLoadIndex);
            getMessagesController().loadMessages(dialog_id, mergeDialogId, false, 1, 0, 0, true, 0, classGuid, 2, 0, ChatObject.isChannel(currentChat), true, lastLoadIndex++);
        }

        if (!inScheduleMode) {
            //            if (userId != 0 && currentUser.bot) {TODO
            //                //机器人
    ...
    public void onFragmentDestroy() {
        ...
        //        getMessagesController().setLastCreatedDialogId(dialog_id, inScheduleMode, false);TODO
        ...
        //        if (currentUser != null) {
        //            getMessagesController().cancelLoadFullUser(currentUser.id);TODO 取消加载
        //        }
        ...
        if (ChatObject.isChannel(currentChat)) {
            //            getMessagesController().startShortPoll(currentChat, true);TODO 不理解这行
        }
        if (textSelectionHelper != null) {
            textSelectionHelper.clear();
        }
        if (chatListItemAniamtor != null) {
            chatListItemAniamtor.onDestroy();
        }
    }
    ...
    public View createView(Context context) {
        ...
        actionBar.setActionBarMenuOnItemClick(new ActionBar.ActionBarMenuOnItemClick() {
            ...
            public void onItemClick(final int id) {
                if (id == -1) {
                    ...
                    //                    AlertsCreator.createClearOrDeleteDialogAlert(ChatActivity.this, id == clear_history, currentChat, currentUser, null != null, (param) -> {
            ...
            searchItem = menu.addItem(0, R.drawable.ic_ab_search)
                             ...
                             .setActionBarMenuItemSearchListener(new ActionBarMenuItem.ActionBarMenuItemSearchListener() {
                                 ...
                                 public void onSearchCollapse() {
                                     ...
                                     //                    getMediaDataController().clearFoundMessageObjects();TODO
        ...
        if (!inScheduleMode) {
            ...
            //            if (currentUser == null || !currentUser.self) {TODO 需要这个，但是缺少属性，在第二步合并对象时要补上
        ...
        fragmentView = new SizeNotifierFrameLayout(context, SharedConfig.smoothKeyboard, parentLayout) {
                        ...
                        if (drawLaterRoundProgressCell != null) {
                            ...
                            if (isRoundVideo) {
                                ...
                                if (drawLaterRoundProgressCell.needDrawTime()) {
                                    drawLaterRoundProgressCell.drawTime(canvas, drawLaterRoundProgressCell.getAlpha());
                                }
                            }
                            canvas.restore();
                        }
            ...
            private void drawChildElement(Canvas canvas, float listTop, ChatMessageCell cell, int type) {
                ...
                if (type == 0) {
                    cell.drawTime(canvas, alpha);
                } else if (type == 1) {
                    cell.drawNamesLayout(canvas, alpha);
                } else {
                    cell.drawCaptionLayout(canvas, (cell.getCurrentPosition().flags & MessageObject.POSITION_FLAG_LEFT) == 0, alpha);
                }
                canvas.restore();
            }
            ...
            protected void dispatchDraw(Canvas canvas) {
                ...
                if (scrimView != null) {
                    ...
                    for (int num = 0; num < count; num++) {
                        ...
                        if (!groupedBackgroundWasDraw && cell != null && scrimGroup != null) {
                            ...
                            float t = (scrimGroup.transitionParams.top + scrimGroup.transitionParams.offsetTop + chatListView.getY());
                            float r = (scrimGroup.transitionParams.right + x + scrimGroup.transitionParams.offsetRight);
                            float b = (scrimGroup.transitionParams.bottom + scrimGroup.transitionParams.offsetBottom + chatListView.getY());
                            ...
                            cell.drawBackground(canvas, (int) l, (int) t, (int) r, (int) b, scrimGroup.transitionParams.pinnedTop, scrimGroup.transitionParams.pinnedBotton);
                            canvas.restore();
            ...
            protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    if (child == chatListView) {
                        ...
                        int h = allHeight;
                        if (SharedConfig.smoothKeyboard) {
                            h -= chatActivityEnterView.getEmojiPadding();
                        }
                        int contentHeightSpec = MeasureSpec.makeMeasureSpec(h - inputFieldHeight + AndroidUtilities.dp(3), MeasureSpec.EXACTLY);
                        child.measure(contentWidthSpec, contentHeightSpec);
        ...
        //region 2.1 emptyViewContainer 没有消息时，显示初始化问候语
        ...
        chatListView = new RecyclerListView(context) {
            ...
            protected void onMeasure(int widthSpec, int heightSpec) {
                ...
                if (lastH != height) {
                    ignoreLayout = true;
                    if (chatListItemAniamtor != null) {
                        chatListItemAniamtor.endAnimations();
                    }
            ...
            public void onDraw(Canvas c) {
                ...
                if (chatListView.isScrollAnimationRunning()) {
                    updateTextureViewPosition(false);
                }
            }
            ...
            protected void dispatchDraw(Canvas canvas) {
                ...
                for (int k = 0; k < 3; k++) {
                    drawingGroups.clear();
                    if (k == 2 && !chatListView.isScrollAnimationRunning()) {
                        continue;
                    }
                    for (int i = 0; i < count; i++) {
                        ...
                        if (child instanceof ChatMessageCell) {
                            ...
                            if (chatListView.isScrollAnimationRunning()) {
                                ...
                                if (child.getTop() > chatListView.getHeight() || child.getTop() + child.getHeight() < 0) {
                                    continue;
                                }
                            }
                            MessageObject.GroupedMessages group = cell.getCurrentMessagesGroup();
            ...
            public boolean drawChild(Canvas canvas, View child, long drawingTime) {
                ...
                if (num == count - 1) {
                    ...
                    if (size > 0) {
                        for (int a = 0; a < size; a++) {
                            ...
                            cell.drawTime(canvas, cell.shouldDrawAlphaLayer() ? cell.getAlpha() : 1f);
                            canvas.restore();
                        }
                        drawTimeAfter.clear();
                    }
                ...
                if (child instanceof ChatMessageCell) {
                    ...
                    if (imageReceiver != null) {
                        ...
                        boolean replaceAnimation = chatListView.isScrollAnimationRunning() || (groupedMessages != null && groupedMessages.transitionParams.backgroundChangeBounds);
        ...
        if (MessagesController.getGlobalMainSettings().getBoolean("view_animations", true)) {
            chatListItemAniamtor = new ChatListItemAnimator(this, chatListView) {
                ...
                public void onAnimationStart() {
                    if (index == -1) {
                        index = getNotificationCenter().setAnimationInProgress(index, allowedNotificationsDuringChatListAnimations);
                    }
            ...
            chatListItemAniamtor.setSupportsChangeAnimations(false);
        }
        ...
        pinnedMessageView.setOnClickListener(v -> {
            wasManualScroll = true;
            scrollToMessageId(currentChat.pinned_msg_id, 0, true, 0, true);
        });
        ...
        closePinned.setOnClickListener(v -> {
            ...
            if (allowPin) {
                ...
                SharedPreferences preferences = MessagesController.getNotificationsSettings(currentAccount);
                preferences.edit().putInt("pin_" + dialog_id, currentChat.pinned_msg_id).commit();
                updatePinnedMessageView(true);
            }
        });
        //endregion
        ...
        reportSpamButton.setOnClickListener(v2 -> AlertsCreator.showBlockReportSpamAlert(ChatActivity.this, dialog_id, currentUser, currentChat, reportSpamButton.getTag(R.id.object_tag) != null, param -> {
            if (param == 0) {
                updateTopPanel(true);
            } else {
                finishFragment();
        ...
        mentionGridLayoutManager = new ExtendedGridLayoutManager(context, 100) {
            ...
            protected Size getSizeForItem(int i) {
                ...
                if (object instanceof BotInlineResult) {
                    BotInlineResult inlineResult = (BotInlineResult) object;
                    if (inlineResult.document != null) {
                        ...
                        for (int b = 0; b < inlineResult.document.attributes.size(); b++) {
                            Document.DocumentAttribute attribute = inlineResult.document.attributes.get(b);
                        ...
                        for (int b = 0; b < inlineResult.content.attributes.size(); b++) {
                            Document.DocumentAttribute attribute = inlineResult.content.attributes.get(b);
                            if (attribute.isImageSize() || attribute.isVideo()) {
                                size.width = attribute.w;
                                size.height = attribute.h;
                                break;
                        ...
                        for (int b = 0; b < inlineResult.thumb.attributes.size(); b++) {
                            Document.DocumentAttribute attribute = inlineResult.thumb.attributes.get(b);
                            if (attribute.isImageSize() || attribute.isVideo()) {
                                size.width = attribute.w;
                                size.height = attribute.h;
                                break;
        ...
        mentionListView.setAdapter(mentionsAdapter = new MentionsAdapter(context, false, dialog_id, new MentionsAdapter.MentionsAdapterDelegate() {
            ...
            public void onContextClick(BotInlineResult result) {
                ...
                if (result.type.equals("video") || result.type.equals("web_player_video")) {
                    ...
                    if (AndroidUtilities.shouldShowUrlInAlert(result.content.url)) {
                        AlertsCreator.showOpenUrlAlert(ChatActivity.this, result.content.url, true, true, true);
                    } else {
        ...
        mentionsAdapter.setChatInfo(currentChat);
        ...
        chatActivityEnterView = new ChatActivityEnterView(getParentActivity(), contentView, this, true) {
            ...
            protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
                ...
                if (chatActivityEnterViewAnimateFromTop != 0 && t != chatActivityEnterViewAnimateFromTop && lastContentViewHeight == contentView.getMeasuredHeight()) {
                    ...
                    int dy = chatActivityEnterViewAnimateFromTop - t;
                    animatedTop = dy;
        ...
        chatActivityEnterView.setChatInfo(currentChat);
        ...
        replyLayout.setOnClickListener(v -> {
            if (forwardingMessages != null && !forwardingMessages.isEmpty()) {
                ...
                HomeActivity fragment = new HomeActivity();//TODO 选择会话转发
        ...
        bottomOverlayChatText.setOnClickListener(view -> {
            ...
            if (currentUser != null && userBlocked) {
                ...
                if (ChatObject.isChannel(currentChat) && !currentChat.isForbidden()) {
                    ...
                    AlertsCreator.createClearOrDeleteDialogAlert(ChatActivity.this, false, currentChat, currentUser, false, (param) -> {
                        getNotificationCenter().removeObserver(ChatActivity.this, NotificationCenter.closeChats);
                        getNotificationCenter().postNotificationName(NotificationCenter.closeChats);
                        finishFragment();
                        getNotificationCenter().postNotificationName(NotificationCenter.needDeleteDialog, dialog_id, currentUser, currentChat, param);
        ...
        bottomOverlayChatText2.setOnClickListener(v -> {
            Bundle args = new Bundle();
            args.putInt("chat_id", currentChat.linked_chat_id);
    ...
    public void onTransitionAnimationStart(boolean isOpen, boolean backward) {
        ...
        if (isOpen) {
            alowedNotifications = new int[]{
                    NotificationCenter.chatInfoDidLoad, NotificationCenter.dialogsNeedReload, NotificationCenter.scheduledMessagesUpdated, NotificationCenter.pinnedMessageDidLoad,
            ...
            if (UserObject.isUserSelf(currentUser)) {
                alowedNotifications = new int[]{
                        NotificationCenter.chatInfoDidLoad, NotificationCenter.dialogsNeedReload, NotificationCenter.scheduledMessagesUpdated,
    ...
    private void openForward() {
        ...
        args.putInt("hasPoll", hasPoll);
        HomeActivity fragment = new HomeActivity();//TODO
    ...
    private void forwardMessages(ArrayList<MessageObject> arrayList, boolean fromMyName, boolean notify, int scheduleDate) {
        ...
        if ((scheduleDate != 0) == inScheduleMode) {
            waitingForSendingMessageLoad = true;
        }
        if (!fromMyName) {
    ...
    /**
     * 根据message创建语音播放列表
     * @param startMessageObject
     * @param playingUnreadMedia
     * @return
     */
    private ArrayList<MessageObject> createVoiceMessagesPlaylist(MessageObject startMessageObject, boolean playingUnreadMedia) {
        ...
        if (messageId != 0) {
            ...
            for (int a = messages.size() - 1; a >= 0; a--) {
                ...
                if (messageObject.getId() > messageId && (messageObject.isVoice() || messageObject.isRoundVideo()) && (!playingUnreadMedia || messageObject.isContentUnread() && !messageObject.isOut())) {
                ...
                if (groupedMessages.messages.isEmpty()) {
                    ...
                    if (index >= 0) {
                        if (chatAdapter != null) {
                            ...
                            if (chatListItemAniamtor != null) {
                                chatListItemAniamtor.groupWillChanged(groupedMessages);
                            }
                        }
                    }
                }
    ...
    private void restartSticker(ChatMessageCell cell) {
        ...
        if (message.isAnimatedEmoji() || MessageObject.isAnimatedStickerDocument(document, message.isOut()) && !SharedConfig.loopStickers) {
        ...
        if (totalItemCount - firstVisibleItem - visibleItemCount <= checkLoadCount && !loading) {
            if (!endReached[0]) {
                ...
                if (messagesByDays.size() != 0) {
                    getMessagesController().loadMessages(dialog_id, mergeDialogId, false, 50, maxMessageId[0], 0, !cacheEndReached[0], minDate[0], classGuid, 0, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
                } else {
                    getMessagesController().loadMessages(dialog_id, mergeDialogId, false, 50, 0, 0, !cacheEndReached[0], minDate[0], classGuid, 0, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
                }
            } else if (mergeDialogId != 0 && !endReached[1]) {
                loading = true;
                waitingForLoad.add(lastLoadIndex);
                getMessagesController().loadMessages(mergeDialogId, 0, false, 50, maxMessageId[1], 0, !cacheEndReached[1], minDate[1], classGuid, 0, 0, false, inScheduleMode, lastLoadIndex++);
            }
        }
        if (visibleItemCount > 0 && !loadingForward && firstVisibleItem <= 10) {
            if (mergeDialogId != 0 && !forwardEndReached[1]) {
                waitingForLoad.add(lastLoadIndex);
                getMessagesController().loadMessages(mergeDialogId, 0, false, 50, minMessageId[1], 0, true, maxDate[1], classGuid, 1, 0, false, inScheduleMode, lastLoadIndex++);
                loadingForward = true;
            } else if (!forwardEndReached[0]) {
                waitingForLoad.add(lastLoadIndex);
                getMessagesController().loadMessages(dialog_id, mergeDialogId, false, 50, minMessageId[0], 0, true, maxDate[0], classGuid, 1, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
                loadingForward = true;
            }
        }
    }
    ...
    private void scrollToLastMessage(boolean pagedown) {
        ...
        if (forwardEndReached[0] && first_unread_id == 0 && startLoadFromMessageId == 0) {
            ...
            waitingForLoad.add(lastLoadIndex);
            getMessagesController().loadMessages(dialog_id, mergeDialogId, false, 30, 0, 0, true, 0, classGuid, 0, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
        }
    }
    ...
    public void updateMessagesVisiblePart(boolean inLayout) {
        ...
        for (int a = 0; a < count; a++) {
            ...
            if (chatListItemAniamtor == null || !chatListItemAniamtor.isRunning()) {
            ...
            if (!foundTextureViewMessage) {
                ...
                if (messageObject != null) {
                    if (checkTextureViewPosition && messageObject.isVideo()) {
                        ...
                        videoPlayerContainer.setTranslationY(-AndroidUtilities.roundMessageSize - 100);
                        fragmentView.invalidate();
                        if ((messageObject.isRoundVideo() || messageObject.isVideo()) && messageObject.eventId == 0 && checkTextureViewPosition && !chatListView.isScrollAnimationRunning()) {
                            MediaController.getInstance().setCurrentVideoVisible(false);
                        }
                    }
                }
        ...
        if (minMessageChild != null && (chatListItemAniamtor == null || !chatListItemAniamtor.isRunning())) {
    ...
    public void scrollToMessageId(int id, int fromMessageId, boolean select, int loadIndex, boolean forceScroll) {
        if (chatListView.isScrollAnimationRunning() || (chatListItemAniamtor != null && chatListItemAniamtor.isRunning()) || getParentActivity() == null) {
            return;
        }
        ...
        if (query) {
            ...
            getMessagesController().loadMessages(loadIndex == 0 ? dialog_id : mergeDialogId, 0, false, AndroidUtilities.isTablet() ? 30 : 20, startLoadFromMessageId, 0, true, 0, classGuid, 3, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
            //emptyViewContainer.setVisibility(View.INVISIBLE);
        } else {
    ...
    private void updateBotButtons() {
        if (headerItem == null || currentUser == null || !currentUser.bot) {
            return;
        }
    ...
    private void updateTitleIcons() {
        ...
        avatarContainer.setTitleIcons(Theme.chat_lockIconDrawable, rightIcon);
        if (muteItem != null) {
            if (rightIcon != null) {
    ...
    private void updateBottomOverlay() {
        ...
        if (currentChat != null) {
            if (ChatObject.isChannel(currentChat) && !(currentChat.isForbidden())) {
                ...
                if (!ChatObject.isNotInChat(currentChat) && !currentChat.megagroup && (currentChat.has_link || true && currentChat.linked_chat_id != 0)) {
    ...
    private void createDeleteMessagesAlert(final MessageObject finalSelectedObject, final MessageObject.GroupedMessages finalSelectedGroup, int loadParticipant) {
        AlertsCreator.createDeleteMessagesAlert(this, currentUser, currentChat, mergeDialogId, finalSelectedObject, selectedMessagesIds, finalSelectedGroup, inScheduleMode, loadParticipant, () -> {
            hideActionMode();
            updatePinnedMessageView(true);
        });
    }
    ...
    private void createMenu(View v, boolean single, boolean listView, float x, float y, boolean searchGroup) {
        ...
        if (single) {
            if (message.messageOwner.action.isPinMessage()) {
        ...
        allowPin = allowPin && message.getId() > 0 && (message.messageOwner.action == null || message.messageOwner.action.isEmpty());
        boolean allowUnpin = message.getDialogId() != mergeDialogId && allowPin && (currentChat.pinned_msg_id == message.getId());
        ...
        if (inScheduleMode ||
                ...
                message.messageOwner.action.isSecureValuesSent() ||
                message.getId() < 0 ||
                bottomOverlayChat != null && bottomOverlayChat.getVisibility() == View.VISIBLE ||
                currentChat != null && (ChatObject.isNotInChat(currentChat) || ChatObject.isChannel(currentChat) && !ChatObject.canPost(currentChat) && !currentChat.megagroup || !ChatObject.canSendMessages(currentChat))) {
            allowChatActions = false;
        ...
        if (single || type < 2 || type == 20) {
            ...
            if (type >= 0 || type == -1 && single && (message.isSending() || message.isEditing())) {
    ...
    //region 机器人
    private void sendBotInlineResult(BotInlineResult result, boolean notify, int scheduleDate) {
    ...
    private void checkBotCommands() {
        ...
        if (currentChat.isRobot()) {
            URLSpanBotCommand.enabled = true;
        } else if (currentChat.isGroup()) {
            for (int a = 0; a < currentChat.participants.participants.size(); a++) {
                Chat.ChatParticipant participant = currentChat.participants.participants.get(a);
                User user = getMessagesController().getUser(participant.user_id);
                if (user != null && user.bot) {
                    URLSpanBotCommand.enabled = true;
                    break;
            ...
            URLSpanBotCommand.enabled = !currentChat.bot_info.isEmpty() && currentChat != null && currentChat.megagroup;
        }
    }
    ...
    public boolean isKeyboardVisible() {
        int keyboardSize = SharedConfig.smoothKeyboard ? 0 : contentView.getKeyboardHeight();
        return keyboardSize > AndroidUtilities.dp(20);
    }
    ...
    private void showScheduledOrNoSoundHint() {
        boolean disableNoSound = UserObject.isUserSelf(currentUser);
        ...
        if (chatAttachAlert == null) {
            ...
            chatAttachAlert.setDelegate(new ChatAttachAlert.ChatAttachViewDelegate() {
                ...
                public void doOnIdle(Runnable runnable) {
                    if (chatListItemAniamtor != null && chatListItemAniamtor.isRunning()) {
                        chatListItemAniamtor.runOnAnimationEnd(runnable);
                    } else {
                        runnable.run();
                    }
                }
    ...
    private void checkAutoDownloadMessages(boolean scrollUp) {
        ...
        for (int a = 0; a < count; a++) {
            ...
            if (!MessageObject.isStickerDocument(document) && !MessageObject.isAnimatedStickerDocument(document, true) && !MessageObject.isGifDocument(document) && !MessageObject.isRoundVideoDocument(document)
                ...
                if (canDownload == 2) {
                    if (!object.shouldEncryptPhotoOrVideo() && object.canStreamVideo()) {
    ...
    private void checkAutoDownloadMessage(MessageObject object) {
        ...
        if (canDownload == 2 || canDownload == 1 && object.isVideo()) {
            if (document != null && !object.shouldEncryptPhotoOrVideo() && object.canStreamVideo()) {
                getFileLoader().loadFile(document, object, 0, 10);
            }
        } else {
            if (document != null) {
    ...
    //region DocumentSelectActivityDelegate
    @Override
    ...
    private void jumpToDate(int date) {
        ...
        if (firstMessage.messageOwner.date >= date && lastMessage.messageOwner.date <= date || lastMessage.messageOwner.date >= date && endReached[0]) {
            ...
            postponedScrollMessageId = 0;
            postponedScrollIsCanceled = false;
            getMessagesController().loadMessages(dialog_id, mergeDialogId, false, 30, 0, date, true, 0, classGuid, 4, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
            floatingDateView.setAlpha(0.0f);
            floatingDateView.setTag(null);
        }
    }
    ...
    private void didPressMessageUrl(CharacterStyle url, boolean longPress, MessageObject messageObject, ChatMessageCell cell) {
        ...
        if (url instanceof URLSpanMono) {
            ...
            if (user != null) {
                //                MessagesController.openChatOrProfileWith(user, null, ChatActivity.this, 0, false);TODO 打开用户详情页面
            }
            ...
            if (longPress) {
                ...
                if (url instanceof URLSpanReplacement
                        && (urlFinal == null
                        || !urlFinal.startsWith("mailto:"))
                        || (punycode = AndroidUtilities.shouldShowUrlInAlert(urlFinal))) {
                    if (!openLinkInternally(urlFinal, messageObject != null ? messageObject.getId() : 0)) {
                        AlertsCreator.showOpenUrlAlert(ChatActivity.this, urlFinal, punycode, true);
                    }
    ...
    private void clearHistory(boolean overwrite) {
        ...
        if (overwrite) {
            ...
            if (startLoadFromMessageIdSaved != 0) {
                ...
                getMessagesController().loadMessages(dialog_id, mergeDialogId, false, AndroidUtilities.isTablet() ? 30 : 20, startLoadFromMessageId, 0, true, 0, classGuid, 3, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
            } else {
                getMessagesController().loadMessages(dialog_id, mergeDialogId, false, AndroidUtilities.isTablet() ? 30 : 20, 0, 0, true, 0, classGuid, 2, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
            }
        } else {
            if (progressView != null) {
                progressView.setVisibility(View.INVISIBLE);
        ...
        if (currentUser != null && currentUser.bot && botUser == null) {
            botUser = "";
            updateBottomOverlay();
        }
    }
...
//                            clipboard.setPrimaryClip(clip); if (exportedMessageLink.link.contains("/c/")) {
...
//                    });
    ...
    public void didReceivedNotification(int id, int account, final Object... args) {
        if (id == NotificationCenter.messagesDidLoad) {
            ...
            onChatOnlineCountDidLoad(id, account, args);
        } else if (id == NotificationCenter.updateInterfaces) {
            onUpdateInterfaces(id, account, args);
        } else if (id == NotificationCenter.didReceiveNewMessages) {
            onDidReceiveNewMessages(id, account, args);
        } else if (id == NotificationCenter.closeChats) {
            onCloseChats(id, account, args);
        } else if (id == NotificationCenter.messagesRead) {
            onMessagesRead(id, account, args);
        } else if (id == NotificationCenter.historyCleared) {
            onHistoryCleared(id, account, args);
        } else if (id == NotificationCenter.messagesDeleted) {
            onMessagesDeleted(id, account, args);
        } else if (id == NotificationCenter.messageReceivedByServer) {
            onMessageReceivedByServer(id, account, args);
        } else if (id == NotificationCenter.messageReceivedByAck) {
            onMessageReceivedByAck(id, account, args);
        } else if (id == NotificationCenter.messageSendError) {
            onMessageSendError(id, account, args);
        } else if (id == NotificationCenter.chatInfoDidLoad) {
            onChatInfoDidLoad(id, account, args);
        } else if (id == NotificationCenter.chatInfoCantLoad) {
            onChatInfoCantLoad(id, account, args);
        } else if (id == NotificationCenter.contactsDidLoad) {
            onContactsDidLoad(id, account, args);
        } else if (id == NotificationCenter.encryptedChatUpdated) {
        } else if (id == NotificationCenter.removeAllMessagesFromDialog) {
            onRemoveAllMessagesFromDialog(id, account, args);
        } else if (id == NotificationCenter.screenshotTook) {
            updateInformationForScreenshotDetector();
        } else if (id == NotificationCenter.blockedUsersDidLoad) {
            onBlockedUsersDidLoad(id, account, args);
        } else if (id == NotificationCenter.fileNewChunkAvailable) {
            onFileNewChunkAvailable(id, account, args);
        } else if (id == NotificationCenter.didCreatedNewDeleteTask) {
            onDidCreatedNewDeleteTask(id, account, args);
        } else if (id == NotificationCenter.messagePlayingDidStart) {
            onMessagePlayingDidStart(id, account, args);
        } else if (id == NotificationCenter.messagePlayingGoingToStop) {
            onMessagePlayingGoingToStop(id, account, args);
        ...
        if (guid == classGuid) {
            ...
            if (postponedScroll) {
                if (load_type == 0 && isCache && messArr.size() < count) {
                    postponedScrollToLastMessageQueryIndex = lastLoadIndex;
                    waitingForLoad.add(lastLoadIndex);
                    getMessagesController().loadMessages(dialog_id, mergeDialogId, false, count, 0, 0, false, 0, classGuid, 0, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
                    return;
                }
            ...
            if (chatListItemAniamtor != null) {
                chatListItemAniamtor.setShouldAnimateEnterFromBottom(false);
            }
            ...
            for (int a = 0, N = messArr.size(); a < N; a++) {
                ...
                if (a == 0 && action.isChatCreate()) {
                    ...
                    break;
                } else if (a < 2 && action.isChatEditPhoto()) {
            ...
            for (int a = 0; a < messArr.size(); a++) {
                ...
                MessageAction action = obj.messageOwner.action;
                if (currentChat != null && currentChat.creator && (action.isChatCreate() || dropPhotoAction != null && action == dropPhotoAction)) {
                    continue;
                }
                ...
                if (load_type != 2 && unreadMessageObject == null && createUnreadMessageAfterId != 0 && (!obj.isOut() || obj.messageOwner.from_scheduled) && messageId >= createUnreadMessageAfterId && (load_type == 1 || prevObj != null || prevObj == null && createUnreadLoading && a == messArr.size() - 1)) {
                    Message dateMsg = new Message();
                    dateMsg.message = "";
                    dateMsg.id = 0;
                    MessageObject dateObj = new MessageObject(currentAccount, dateMsg, false);
            ...
            if (load_type == 1) {
                if (messArr.size() != count && (!isCache || forwardEndReached[loadIndex])) {
                    forwardEndReached[loadIndex] = true;
                    if (loadIndex != 1) {
                        first_unread_id = 0;
                        last_message_id = 0;
                ...
                if (messArr.size() < count && load_type != 3 && load_type != 4) {
                    if (isCache) {
                        if (loadIndex == 1 && mergeDialogId != 0 && isEnd) {
                            endReached[loadIndex] = true;
                        }
                        if (load_type != 2) {
                            cacheEndReached[loadIndex] = true;
                ...
                if (chatListView != null && chatScrollHelper != null) {
                    if (first || scrollToTopOnResume || forceScrollToTop) {
                        ...
                        if (newRowsCount != 0) {
                            ...
                            if (loadingUpRow != -1 && chatAdapter.loadingUpRow == -1) {
                        ...
                        } else if (chatAdapter.loadingUpRow != -1 && endReached[loadIndex] && (loadIndex == 0 && mergeDialogId == 0 || loadIndex == 1)) {
            ...
            if (messages.isEmpty() && currentUser != null && currentUser.bot && botUser == null) {
                botUser = "";
                updateBottomOverlay();
            }
    ...
    public void onEmojiDidLoad(int id, int account, final Object... args) {
        if (chatListView != null) {
    ...
    public void onChatOnlineCountDidLoad(int id, int account, final Object... args) {
        Integer chatId = (Integer) args[0];
        if (currentChat == null || currentChat.id != chatId) {
            return;
        }
        currentChat.online_count = (Integer) args[1];
        if (avatarContainer != null) {
            avatarContainer.updateOnlineCount();
            avatarContainer.updateSubtitle();
        }
    }
    public void onUpdateInterfaces(int id, int account, final Object... args) {
        ...
        if ((updateMask & MessagesController.UPDATE_MASK_READ_DIALOG_MESSAGE) != 0) {
            if (bottomOverlayChatText2 != null && ChatObject.isChannel(currentChat) && !currentChat.megagroup && -currentChat.linked_chat_id != 0) {
                bottomOverlayChatText2.updateCounter();
            }
        }
    ...
    public void onDidReceiveNewMessages(int id, int account, final Object... args) {
        //收到新消息
        long did = (Long) args[0];
        if (did == dialog_id) {
            ...
            for (int a = 0; a < arr.size(); a++) {
                ...
                if (messageObject.isOut()) {
                    ...
                    if (currentChat != null && currentChat.slowmode_enabled && messageObject.isSent() && !inScheduleMode) {
                        int date = messageObject.messageOwner.date + currentChat.slowmode_seconds;
                        int currentTime = getConnectionsManager().getCurrentTime();
                        if (date > getConnectionsManager().getCurrentTime()) {
                            currentChat.slowmode_next_send_date = Math.max(currentChat.slowmode_next_send_date, Math.min(currentTime + currentChat.slowmode_seconds, date));
                            if (chatActivityEnterView != null) {
                ...
                if (currentChat != null) {
                    if (messageObject.messageOwner.action.isChatDeleteUser() && messageObject.messageOwner.action.user_id == currentUserId ||
                            messageObject.messageOwner.action.isChatAddUser() && messageObject.messageOwner.action.users.contains(currentUserId)) {
                ...
                if (messageObject.messageOwner.reply_to_msg_id != 0 && messageObject.replyMessageObject == null) {
                    ...
                    if (messageObject.messageOwner.action.isPinMessage()) {
            ...
            if (!forwardEndReached[0]) {
                ...
                for (int a = 0; a < arr.size(); a++) {
                    ...
                    MessageAction action = obj.messageOwner.action;
                    if (currentChat != null && currentChat.megagroup && (action.isChatAddUser() || action.isChatDeleteUser())) {
                    ...
                    if (currentChat != null && currentChat.creator && (action.isChatCreate() || action.isChatEditPhoto() && messages.size() < 4)) {
                        continue;
                    }
                    obj.checkLayout();
    ...
    public void onCloseChats(int id, int account, final Object... args) {
    ...
    public void onMessagesRead(int id, int account, final Object... args) {
        if (inScheduleMode) {
            return;
        }
        ...
        if (inbox != null) {
            if (bottomOverlayChatText2 != null
                    && ChatObject.isChannel(currentChat)
                    && !currentChat.megagroup
                    && -currentChat.linked_chat_id != 0) {
                bottomOverlayChatText2.updateCounter();
            }
    ...
    public void onHistoryCleared(int id, int account, final Object... args) {
        long did = (Long) args[0];
        if (did != dialog_id) {
            return;
        }
        ...
        for (int b = 0; b < messages.size(); b++) {
            ...
            if (currentChat.pinned_msg_id == mid) {
                pinnedMessageObject = null;
                currentChat.pinned_msg_id = 0;
                getMessagesStorage().updateChatPinnedMessage(currentChat.id, 0);
                updatePinnedMessageView(true);
            }
            messages.remove(b);
            b--;
        ...
        if (messages.isEmpty()) {
            if (!endReached[0] && !loading) {
                ...
                waitingForLoad.add(lastLoadIndex);
                getMessagesController().loadMessages(dialog_id, mergeDialogId, false, 30, 0, 0, !cacheEndReached[0], minDate[0], classGuid, 0, 0, ChatObject.isChannel(currentChat), inScheduleMode, lastLoadIndex++);
                loading = true;
            } else {
                if (botButtons != null) {
                    botButtons = null;
    ...
    public void onMessagesDeleted(int id, int account, final Object... args) {
        boolean scheduled = (Boolean) args[2];
        if (scheduled != inScheduleMode) {
            return;
        }
        ...
        for (int a = 0; a < size; a++) {
            ...
            if (loadIndex == 0 && currentChat.pinned_msg_id == ids) {
                pinnedMessageObject = null;
                currentChat.pinned_msg_id = 0;
            ...
            for (int a = 0; a < newGroups.size(); a++) {
                ...
                if (chatListItemAniamtor != null) {
                    if (groupedMessages.messages.size() == 1) {
                        chatListItemAniamtor.groupWillTransformToSingleMessage(groupedMessages);
                    } else {
                        chatListItemAniamtor.groupWillChanged(groupedMessages);
                    }
                }
    ...
    public void onMessageSendError(int id, int account, final Object... args) {
        Integer msgId = (Integer) args[0];
        MessageObject obj = messagesDict[0].get(msgId);
        if (obj != null) {
    ...
    public void onChatInfoDidLoad(int id, int account, final Object... args) {
        ...
        if (currentChat != null && chatFull.id == currentChat.id) {
            if (currentChat.isChannel()) {
                ...
                if (chatFull.participants == null) {
                    chatFull.participants = currentChat.participants;
                }
            }
            if (chatActivityEnterView != null) {
                chatActivityEnterView.setChatInfo(currentChat);
            ...
            if (currentChat.isGroup()) {
                ...
                botsCount = 0;
                URLSpanBotCommand.enabled = !currentChat.bot_info.isEmpty() && currentChat != null && currentChat.megagroup;
                botsCount = currentChat.bot_info.size();
                for (int a = 0; a < currentChat.bot_info.size(); a++) {
                    BotInfo bot = currentChat.bot_info.get(a);
            ...
            if (!inScheduleMode && ChatObject.isChannel(currentChat)
                    && mergeDialogId == 0 && currentChat.migrated_from_chat_id != 0) {
                mergeDialogId = -currentChat.migrated_from_chat_id;
                maxMessageId[1] = currentChat.migrated_from_max_id;
                if (chatAdapter != null) {
                    chatAdapter.notifyDataSetChanged();
    ...
    public void onChatInfoCantLoad(int id, int account, final Object... args) {
    ...
    public void onContactsDidLoad(int id, int account, final Object... args) {
        updateTopPanel(true);
        if (avatarContainer != null) {
            avatarContainer.updateSubtitle();
        }
    }
    public void onRemoveAllMessagesFromDialog(int id, int account, final Object... args) {
        long did = (Long) args[0];
        if (dialog_id == did) {
            clearHistory((Boolean) args[1]);
        }
    }
    public void onBlockedUsersDidLoad(int id, int account, final Object... args) {
        if (currentUser != null) {
    ...
    public void onFileNewChunkAvailable(int id, int account, final Object... args) {
    ...
    public void onDidCreatedNewDeleteTask(int id, int account, final Object... args) {
    ...
    public void onMessagePlayingDidStart(int id, int account, final Object... args) {
        MessageObject messageObject = (MessageObject) args[0];
    ...
    public void onMessagePlayingGoingToStop(int id, int account, final Object... args) {
    ...
    public void messagePlayingProgressDidChanged(int id, int account, final Object... args) {
    ...
    public void didUpdateReactions(int id, int account, final Object... args) {
        long did = (Long) args[0];
    ...
    public void didVerifyMessagesStickers(int id, int account, final Object... args) {
    ...
    public void updateMessageMedia(int id, int account, final Object... args) {
    ...
    public void replaceMessagesObjects(int id, int account, final Object... args) {
        long did = (long) args[0];
        if (did != dialog_id && did != mergeDialogId) {
            return;
        }
    ...
    public void notificationsSettingsUpdated(int id, int account, final Object... args) {
        updateTitleIcons();
        if (ChatObject.isChannel(currentChat)) {
            updateBottomOverlay();
        }
    }
    public void replyMessagesDidLoad(int id, int account, final Object... args) {
        long did = (Long) args[0];
        if (did == dialog_id) {
            updateVisibleRows();
        }
    }
    public void pinnedMessageDidLoad(int id, int account, final Object... args) {
        MessageObject message = (MessageObject) args[0];
        if (message.getDialogId() == dialog_id && currentChat.pinned_msg_id == message.getId()) {
            pinnedMessageObject = message;
            loadingPinnedMessage = 0;
            updatePinnedMessageView(true);
        }
    }
    public void didReceivedWebpages(int id, int account, final Object... args) {
    ...
    public void didReceivedWebpagesInUpdates(int id, int account, final Object... args) {
        if (foundWebPage != null) {
            LongSparseArray<MessageMedia.WebPage> hashMap = (LongSparseArray<MessageMedia.WebPage>) args[0];
    ...
    public void messagesReadContent(int id, int account, final Object... args) {
    ...
    public void botInfoDidLoad(int id, int account, final Object... args) {
        int guid = (Integer) args[1];
        if (classGuid == guid) {
            BotInfo info = (BotInfo) args[0];
            ...
            if (chatAdapter != null) {
                ...
                chatAdapter.updateRowsInternal();
                if (prevRow == -1 && chatAdapter.botInfoRow != -1) {
                    chatAdapter.notifyItemInserted(chatAdapter.botInfoRow);
                } else if (prevRow != -1 && chatAdapter.botInfoRow == -1) {
    ...
    public void botKeyboardDidLoad(int id, int account, final Object... args) {
    ...
    public void chatSearchResultsAvailable(int id, int account, final Object... args) {
    ...
    public void chatSearchResultsLoading(int id, int account, final Object... args) {
        if (classGuid == (Integer) args[0]) {
            if (searchItem != null) {
    ...
    public void didUpdateMessagesViews(int id, int account, final Object... args) {
    ...
    public void peerSettingsDidLoad(int id, int account, final Object... args) {
        long did = (Long) args[0];
        if (did == dialog_id || currentUser != null && currentUser.id == did) {
            updateTopPanel(!paused);
        }
    }
    public void newDraftReceived(int id, int account, final Object... args) {
        long did = (Long) args[0];
        if (did == dialog_id) {
            applyDraftMaybe(true);
        }
    }
    public void userInfoDidLoad(int id, int account, final Object... args) {
    ...
    public void didSetNewWallpapper(int id, int account, final Object... args) {
        if (fragmentView != null) {
            contentView.setBackgroundImage(Theme.getCachedWallpaper(), Theme.isWallpaperMotion());
            progressView2.getBackground().setColorFilter(Theme.colorFilter);
            if (emptyView != null) {
    ...
    public void didApplyNewTheme(int id, int account, final Object... args) {
    ...
    public void goingToPreviewTheme(int id, int account, final Object... args) {
        if (chatLayoutManager != null) {
    ...
    public void channelRightsUpdated(int id, int account, final Object... args) {
    ...
    public void updateMentionsCount(int id, int account, final Object... args) {
    ...
    public void audioRecordTooShort(int id, int account, final Object... args) {
    ...
    public void videoLoadingStateChanged(int id, int account, final Object... args) {
        if (chatListView != null) {
    ...
    public void scheduledMessagesUpdated(int id, int account, final Object... args) {
        long did = (Long) args[0];
        if (dialog_id == did) {
    ...
    public void diceStickersDidLoad(int id, int account, final Object... args) {
        if (chatListView == null) {
            return;
        }
        int count = chatListView.getChildCount();
    ...
    private void updatePinnedMessageView(boolean animated) {
        ...
        SharedPreferences preferences = MessagesController.getNotificationsSettings(currentAccount);
        if (pinned_msg_id == 0 || pinned_msg_id == preferences.getInt("pin_" + dialog_id, 0) || actionBar != null && (actionBar.isActionModeShowed() || actionBar.isSearchFieldVisible())) {
    ...
    public boolean canScheduleMessage() {
        return bottomOverlayChat == null || bottomOverlayChat.getVisibility() != View.VISIBLE;
    }
     ...
     * @param scheduleDate
     */
    public void sendContact(User user, boolean notify, int scheduleDate) {
    ...
    public class ChatActivityAdapter extends RecyclerAnimationScrollHelper.AnimatableAdapter {
        ...
        private void updateRowsInternal() {
            ...
            if (!messages.isEmpty()) {
                ...
                if (currentUser != null && currentUser.bot && !inScheduleMode && botInfo.size() > 0 && botInfo.get(currentUser.id).description != null) {
                    botInfoRow = rowCount++;
                } else {
                    botInfoRow = -1;
                }
...
//                        presentFragment(fragment);TODO
    ...
    public ArrayList<ThemeDescription> getThemeDescriptions() {
        ...
        themeDescriptions.add(new ThemeDescription(bottomOverlayChatText2, 0, null, null, null, null, Theme.key_chat_fieldOverlayText));
        ...
        themeDescriptions.add(new ThemeDescription(bottomOverlayChatText2, 0, null, null, null, null, Theme.key_chat_goDownButtonCounterBackground));
        ...
        themeDescriptions.add(new ThemeDescription(bottomOverlayChatText2, 0, null, null, null, null, Theme.key_chat_messagePanelBackground));
