...
import static com.demo.chat.model.Message.MESSAGE_FLAG_HAS_VIEWS;
...
//    private messages_Dialogs loadDialogsByIds(String ids, ArrayList<Integer> usersToLoad, ArrayList<Integer> chatsToLoad, ArrayList<Integer> encryptedToLoad) throws Exception {
...
//                                message.action instanceof TL_messageActionPinMessage ||
//                                        message.action instanceof TL_messageActionPaymentSent ||
//                                        message.action instanceof TL_messageActionGameScore)) {
//                            if (!cursor.isNull(13)) {
...
//                                            message.replyMessage.flags |= MESSAGE_FLAG_MEGAGROUP;
//                                        }
...
//                    addUsersAndChatsFromMessage(message, usersToLoad, chatsToLoad);
//
//                    Message owner = replyMessageOwners.get(message.dialog_id);
//                    if (owner != null) {
//                        owner.replyMessage = message;
//                        message.dialog_id = owner.dialog_id;
//                        if (MessageObject.isMegagroup(owner)) {
//                            owner.replyMessage.flags |= MESSAGE_FLAG_MEGAGROUP;
//                        }
//                    }
//                }
//            }
...
//    }
    ...
    private void calcUnreadCounters(boolean apply) {
        try {
            ...
            final ArrayList<User> users = new ArrayList<>();
            final ArrayList<User> encUsers = new ArrayList<>();
            final ArrayList<Chat> chats = new ArrayList<>();
            ArrayList<Integer> usersToLoad = new ArrayList<>();
            ArrayList<Integer> chatsToLoad = new ArrayList<>();
            ArrayList<Integer> encryptedToLoad = new ArrayList<>();
            ...
            SparseArray<User> encUsersDict = new SparseArray<>();
...
//                        EncryptedChat encryptedChat = encryptedChats.get(a);
...
//                    if (chat.migrated_to instanceof TL_inputChannel || ChatObject.isNotInChat(chat)) {
...
//                        if (did > 0) {
//                            for (int i = 0; i < 2; i++) {
//                                SparseArray<User> dict = i == 0 ? usersDict : encUsersDict;
...
//                ArrayList<Integer> usersToLoad = new ArrayList<>();
//                HashMap<Integer, InputPeer> usersToLoadMap = new HashMap<>();
//                ArrayList<Integer> chatsToLoad = new ArrayList<>();
//                HashMap<Integer, InputPeer> chatsToLoadMap = new HashMap<>();
//                ArrayList<Integer> dialogsToLoad = new ArrayList<>();
//                HashMap<Integer, InputPeer> dialogsToLoadMap = new HashMap<>();
...
//                    TL_dialogFilter newFilter = (TL_dialogFilter) vector.objects.get(a);
...
//                        }
//                        for (int c = 0; c < 2; c++) {
//                            ArrayList<InputPeer> fromArray = c == 0 ? newFilter.include_peers : newFilter.exclude_peers;
...
//                                    InputPeer peer = newFilter.pinned_peers.get(b);
//                                    Integer id;
//                                    if (peer.user_id != 0) {
//                                        id = peer.user_id;
//                                    } else {
...
//                                InputPeer peer = fromArray.get(b);
//                                if (peer.user_id != 0) {
...
//                        Dialog dialog = dialogs.dialogs.get(a);
...
//    private void processLoadedFilterPeersInternal(messages_Dialogs pinnedDialogs, messages_Dialogs pinnedRemoteDialogs, ArrayList<User> users, ArrayList<Chat> chats, ArrayList<MessagesController.DialogFilter> filtersToSave, SparseArray<MessagesController.DialogFilter> filtersToDelete, ArrayList<Integer> filtersOrder, HashMap<Integer, HashSet<Integer>> filterDialogRemovals, HashMap<Integer, HashSet<Integer>> filterUserRemovals, HashSet<Integer> filtersUnreadCounterReset) {
...
//
//                            try {
//                                if (message.reply_to_msg_id != 0 && message.action.isPinMessage()) {
//                                    if (!cursor.isNull(6)) {
...
//                                            replyMessages.add(messageId);
//                                        }
//                                        ArrayList<Message> arrayList = replyMessageOwners.get(message.reply_to_msg_id);
//                                        if (arrayList == null) {
//                                            arrayList = new ArrayList<>();
...
//                                addUsersAndChatsFromMessage(message, usersToLoad, chatsToLoad);
//
//                                ArrayList<Message> arrayList = replyMessageOwners.get(message.id);
//                                if (arrayList != null) {
//                                    for (int a = 0; a < arrayList.size(); a++) {
//                                        Message m = arrayList.get(a);
//                                        m.replyMessage = message;
//                                        if (MessageObject.isMegagroup(m)) {
//                                            m.replyMessage.flags |= MESSAGE_FLAG_MEGAGROUP;
//                                        }
//                                    }
//                                }
//                            }
...
//        });
    ...
    public void putWallpapers(final ArrayList<WallPaper> wallPapers, int action) {
//        storageQueue.postRunnable(() -> {
//            try {
//                if (action == 1) {
...
//                    TL_wallPaper wallPaper = (TL_wallPaper) wallPapers.get(a);
//                    state.requery();
...
//                final ArrayList<TL_wallPaper> wallPapers = new ArrayList<>();
//                while (cursor.next()) {
//                    NativeByteBuffer data = cursor.byteBufferValue(0);
//                    if (data != null) {
//                        TL_wallPaper wallPaper = (TL_wallPaper) WallPaper.TLdeserialize(data, data.readInt32(false), false);
//                        data.reuse();
//                        if (wallPaper != null) {
...
//        });
    ...
    private boolean addFilesToDelete(Message message, ArrayList<File> filesToDelete, boolean forceCache) {
        if (message == null) {
            return false;
        }
        if (message.media.isPhoto() && message.media.photo != null) {
            for (int a = 0, N = message.media.photo.sizes.size(); a < N; a++) {
                PhotoSize photoSize = message.media.photo.sizes.get(a);
            ...
            return true;
        } else if (message.media.isDocument() && message.media.document != null) {
            ...
            for (int a = 0, N = message.media.document.thumbs.size(); a < N; a++) {
                PhotoSize photoSize = message.media.document.thumbs.get(a);
...
//    public void resetDialogs(final messages_Dialogs dialogsRes, final int messagesCount, final int seq, final int newPts, final int date, final int qts, final LongSparseArray<Dialog> new_dialogs_dict, final LongSparseArray<MessageObject> new_dialogMessage, final Message lastMessage, final int dialogsCount) {
//        storageQueue.postRunnable(() -> {
//            try {
//                int maxPinnedNum = 0;
//
...
//                    Dialog dialog = dialogsRes.dialogs.get(dialogsCount + a);
...
//                    Photo photo = photos.photos.get(a);
                        ...
                        if (message.media != null) {
                            if (!addFilesToDelete(message, filesToDelete, true)) {
                                ...
                                if (message.media.document != null) {
                                    ...
                                    message.media.photo = new MessageMedia.Photo();
                                }
                            }
...
//                        if ((message.flags & MESSAGE_FLAG_HAS_VIEWS) != 0) {
...
//                }
    ...
    public void updateMessageReactions(long dialogId, int msgId, int channelId, MessageReactions reactions) {
//        storageQueue.postRunnable(() -> {
//            try {
//                database.beginTransaction();
//                long mid = msgId;
        ...
        storageQueue.postRunnable(() -> {
            try {
                ...
                if (cursor.next()) {
                    NativeByteBuffer data = cursor.byteBufferValue(0);
                    if (data != null) {
                        info = Chat.TLdeserialize(data, data.readInt32(false), false);
                        data.reuse();
                ...
                cursor.dispose();
                if (info instanceof Chat) {
                    info.participants = participants;
                    final Chat finalInfo = info;
...
//                    ChannelParticipant participant = participants.get(a);
//                    state.requery();
...
//    public void loadUserInfo(User user, final boolean force, int classGuid) {
//        if (user == null) {
//            return;
//        }
//        storageQueue.postRunnable(() -> {
...
//                }
//                cursor.dispose();
//                if (info instanceof UserFull) {
//                    info.pinned_msg_id = messageId;
//                    info.flags |= 64;
//
//                    final UserFull finalInfo = info;
...
//                        }
//                    } else if (what == 0) {
//                        for (ChatParticipant part : info.participants.participants) {
...
//                }
//                cursor.dispose();
//                result[0] = info instanceof TL_channelFull && info.migrated_from_chat_id != 0;
...
//    private ChatFull loadChatInfoInternal(final int chat_id, final boolean force, final boolean byChannelUsers) {
//        MessageObject pinnedMessageObject = null;
//        ChatFull info = null;
//        ArrayList<User> loadedUsers = new ArrayList<>();
//        try {
...
//                for (int a = 0; a < info.participants.participants.size(); a++) {
//                    ChatParticipant c = info.participants.participants.get(a);
...
//                    BotInfo botInfo = info.bot_info.get(a);
//                    if (usersToLoad.length() != 0) {
//                        usersToLoad.append(",");
//                    }
...
//            }
//            if (info != null && info.pinned_msg_id != 0) {
//                pinnedMessageObject = getMediaDataController().loadPinnedMessage(-chat_id, info instanceof TL_channelFull ? chat_id : 0, info.pinned_msg_id, false);
//            }
//        } catch (Exception e) {
//            FileLog.e(e);
//        } finally {
...
//                    TL_contact contact = contactsCopy.get(a);
//                    state.requery();
                        ...
                        if (message.reply_to_msg_id != 0 || message.reply_to_random_id != 0) {
                            ...
                            if (message.replyMessage == null) {
                                if (message.reply_to_msg_id != 0) {
                                    ...
                                    ArrayList<Message> messages = replyMessageOwners.get(message.reply_to_msg_id);
                                    if (messages == null) {
                                    ...
                                    ArrayList<Message> messages = replyMessageRandomOwners.get(message.reply_to_random_id);
                                    if (messages == null) {
                                        messages = new ArrayList<>();
                            ...
                            if (arrayList != null) {
                                for (int a = 0; a < arrayList.size(); a++) {
                                    ...
                                    if (MessageObject.isMegagroup(object)) {
                                        object.replyMessage.flags |= MESSAGE_FLAG_MEGAGROUP;
                                    }
                                }
                            }
                        } else {
...
//        return null;
            ...
            try {
                ...
                if (id != null) {
                    ...
                    if (cursor.next()) {
                        NativeByteBuffer data = cursor.byteBufferValue(0);
                        if (data != null) {
                            MessageMedia file = MessageMedia.TLdeserialize(data, data.readInt32(false), false);
                            data.reuse();
                            if (file .isDocument()) {
                                result[0] = ( file).document;
    ...
    public void putSentFile(final String path, final Media file, final int type, String parent) {
...
//                    if (file instanceof MessageMedia.Photo) {
...
//    public void updateChatDefaultBannedRights(int chatId, TL_chatBannedRights rights, int version) {
...
//                    NativeByteBuffer data = cursor.byteBufferValue(0);
//                    if (data != null) {
//                        chat = Chat.TLdeserialize(data, data.readInt32(false), false);
//                        data.reuse();
//                    }
//                }
//                cursor.dispose();
...
//    }
    ...
    private void putUsersAndChatsInternal(final ArrayList<User> users, final ArrayList<Chat> chats, final boolean withTransaction) {
        try {
            if (withTransaction) {
                database.beginTransaction();
            }
...
//            if ((message.media instanceof TL_messageMediaPhoto || MessageObject.isGifMessage(message)) && message.ttl > 0 && message.ttl <= 60 ||
//                    MessageObject.isVoiceMessage(message) ||
//                    MessageObject.isVideoMessage(message) ||
//                    MessageObject.isRoundVideoMessage(message)) {
//                return 1;
//            } else if (message.media instanceof TL_messageMediaPhoto || MessageObject.isVideoMessage(message)) {
//                return 0;
//            }
...
//                Dialog dialog = difference.dialog;
//                dialog.id = did;
//                dialog.flags = 1;
...
//    }
    ...
    private void putMessagesInternal(final ArrayList<Message> messages, final boolean withTransaction, final boolean doNotUpdateDialogDate, final int downloadMask, boolean ifNoLastMessage, boolean scheduled) {
        try {
//            if (scheduled) {
//                if (withTransaction) {
//                    database.beginTransaction();
...
//                    }
//
//                    if (!(message.action instanceof TL_messageActionHistoryClear) && (!MessageObject.isOut(message) || message.from_scheduled) && (message.id > 0 || MessageObject.isUnread(message))) {
...
//                    if (message.action instanceof TL_messageEncryptedAction && !(message.action.encryptedAction instanceof TL_decryptedMessageActionSetMessageTTL || message.action.encryptedAction instanceof TL_decryptedMessageActionScreenshotMessages)) {
//                        updateDialog = false;
//                    } else if (message.out) {
...
//                        }
//                        TL_messageMediaPoll mediaPoll = (TL_messageMediaPoll) message.media;
//                        state_polls.requery();
...
//            }
    ...
    public void putMessages(final ArrayList<Message> messages, final boolean withTransaction, boolean useQueue, final boolean doNotUpdateDialogDate, final int downloadMask, boolean scheduled) {
        putMessages(messages, withTransaction, useQueue, doNotUpdateDialogDate, downloadMask, false, scheduled);
    }

    public void putMessages(final ArrayList<Message> messages, final boolean withTransaction, boolean useQueue, final boolean doNotUpdateDialogDate, final int downloadMask, final boolean ifNoLastMessage, boolean scheduled) {
        if (messages.size() == 0) {
            return;
        }
        if (useQueue) {
    ...
    public void markMessageAsSendError(final Message message, boolean scheduled) {
        storageQueue.postRunnable(() -> {
            try {
                long messageId = message.id;
                if (message.to_id != 0) {
    ...
    private void updateUsersInternal(final ArrayList<User> users, final boolean onlyStatus, final boolean withTransaction) {
        try {
            if (onlyStatus) {
                if (withTransaction) {
                    database.beginTransaction();
...
//                    TL_dialogFolder dialogFolder = new TL_dialogFolder();
//                    if (!cursor.isNull(16)) {
//                        NativeByteBuffer data = cursor.byteBufferValue(16);
//                        if (data != null) {
//                            dialogFolder.folder = TL_folder.TLdeserialize(data, data.readInt32(false), false);
//                        } else {
//                            dialogFolder.folder = new TL_folder();
...
//        }
    ...
    public void replaceMessageIfExists(final Message message, ArrayList<User> users, ArrayList<Chat> chats, boolean broadcast) {
        if (message == null) {
            return;
        }
//        storageQueue.postRunnable(() -> {
...
//    public void putMessages(final messages_Messages messages, final long dialog_id, final int load_type, final int max_id, final boolean createDialog, final boolean scheduled) {
//        storageQueue.postRunnable(() -> {
//            try {
//                if (scheduled) {
...
//                                    if (oldMessage.media instanceof TL_messageMediaPhoto && message.media instanceof TL_messageMediaPhoto && oldMessage.media.photo != null && message.media.photo != null) {
//                                        sameMedia = oldMessage.media.photo.id == message.media.photo.id;
//                                    } else if (oldMessage.media instanceof TL_messageMediaDocument && message.media instanceof TL_messageMediaDocument && oldMessage.media.document != null && message.media.document != null) {
...
//                            state_media.bindByteBuffer(5, data);
//                            state_media.step();
//                        } else if (message instanceof TL_messageService && message.action instanceof TL_messageActionHistoryClear) {
//                            try {
...
//            final LongSparseArray<DraftMessage> drafts = getMediaDataController().getDrafts();
...
//    private void putDialogsInternal(final messages_Dialogs dialogs, int check) {
//        try {
//            database.beginTransaction();
//            final LongSparseArray<Message> new_dialogMessage = new LongSparseArray<>(dialogs.messages.size());
...
//                        state_messages.bindInteger(9, (message.flags & MESSAGE_FLAG_HAS_VIEWS) != 0 ? message.views : 0);
//                        int flags = 0;
//                        if (message.stickerVerified == 0) {
//                            flags |= 1;
//                        } else if (message.stickerVerified == 2) {
...
//                        TL_dialogFolder dialogFolder = (TL_dialogFolder) dialog;
...
//    }
//
//    public void setDialogsFolderId(final ArrayList<TL_folderPeer> peers, ArrayList<TL_inputFolderPeer> inputPeers, long dialogId, int folderId) {
...
//                        TL_folderPeer folderPeer = peers.get(a);
...
//                        TL_inputFolderPeer folderPeer = inputPeers.get(a);
//                        long did = DialogObject.getPeerDialogId(folderPeer.peer);
//                        state.requery();
//                        state.bindInteger(1, folderPeer.folder_id);
//                        state.bindInteger(2, 0);
...
//    }
    ...
    public ArrayList<User> getUsers(final ArrayList<Integer> uids) {
        ArrayList<User> users = new ArrayList<>();
        try {
    ...
    public Chat getChat(final int chat_id) {
        Chat chat = null;
        try {
            ArrayList<Chat> chats = new ArrayList<>();
